...
import { getCommissionsForArtist, updateCommissionStatus } from '../services/commissions'
import ArtworkCard from '../components/ArtworkCard.vue'

const { usuario } = useAuth()

const obras = ref([])
const perfil = ref({ bio: '', location: '', website: '', instagram: '', acceptsCommissions: false })
const encargos = ref([])
const pestana = ref('obras') // obras | perfil | encargos
...
onMounted(async () => {
  ...
  const [misObras, miPerfil, misEncargos] = await Promise.all([
    getArtworksByArtist(usuario.value.email),
    getProfile(usuario.value.email),
    getCommissionsForArtist(usuario.value.email),
  ])
...
const ESTADO_LABELS = { pending: 'Pendiente', accepted: 'Aceptado', rejected: 'Rechazado' }
</script>
...
<template>
  <div class="mi-perfil">
    ...
    <div class="perfil-header">
      <img v-if="usuario?.photoURL" :src="usuario.photoURL" alt="avatar" class="avatar-img" />
      <div v-else class="avatar-placeholder">{{ usuario?.email?.[0].toUpperCase() }}</div>
      <div>
        <h1>{{ usuario?.displayName || usuario?.email }}</h1>
        <p class="email">{{ usuario?.email }}</p>
      </div>
      <RouterLink to="/subir-obra" class="btn-primary" style="margin-left: auto">+ Subir obra</RouterLink>
    </div>
    ...
    <div class="tabs">
      ...
      <button :class="{ activo: pestana === 'perfil' }" @click="pestana = 'perfil'">Perfil artístico</button>
      <button :class="{ activo: pestana === 'encargos' }" @click="pestana = 'encargos'">
    ...
    <div v-if="pestana === 'obras'">
      <div v-if="!obras.length" class="estado-vacio">Aún no has subido ninguna obra. <RouterLink to="/subir-obra">Subir la primera →</RouterLink></div>
      <div v-else class="grid-obras">
        <div v-for="obra in obras" :key="obra.id" class="obra-wrapper">
          <ArtworkCard :artwork="obra" />
          <button class="btn-borrar" @click="borrarObra(obra.id)" title="Eliminar obra">✕</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="pestana === 'perfil'" class="form-perfil">
      <div class="campo">
        <label>Bio</label>
        <textarea v-model="perfil.bio" placeholder="Cuéntanos quién eres y qué haces..." rows="4" />
      </div>
      <div class="campo">
      ...
      <button class="btn-primary" :disabled="guardandoPerfil" @click="guardarPerfil">
        {{ guardandoPerfil ? 'Guardando...' : perfilGuardado ? '✓ Guardado' : 'Guardar perfil' }}
      </button>
    </div>

    <!-- Tab: Encargos -->
    <div v-if="pestana === 'encargos'">
      <div v-if="!encargos.length" class="estado-vacio">No tienes solicitudes de encargo.</div>
      <div v-else class="lista-encargos">
        <div v-for="e in encargos" :key="e.id" class="encargo-card">
          <div class="encargo-info">
            <span class="encargo-cliente">{{ e.clientName }} — {{ e.clientEmail }}</span>
            <p class="encargo-desc">{{ e.description }}</p>
            <span v-if="e.budget" class="encargo-budget">Presupuesto: {{ e.budget }} €</span>
          </div>
          <div class="encargo-acciones">
            <span class="badge-estado" :class="e.status">{{ ESTADO_LABELS[e.status] }}</span>
            <template v-if="e.status === 'pending'">
              <button class="btn-aceptar" @click="cambiarEstadoEncargo(e.id, 'accepted')">Aceptar</button>
              <button class="btn-rechazar" @click="cambiarEstadoEncargo(e.id, 'rejected')">Rechazar</button>
            </template>
          </div>
        </div>
      </div>
...
h1 { font-family: var(--font-display); font-size: 1.8rem; font-weight: 700; color: var(--c-text); }
.email { font-size: 0.85rem; color: var(--c-text-muted); }
...
.tabs button {
  background: transparent; border: none; padding: 0.6rem 1.1rem;
  color: var(--c-text-muted); font-family: var(--font-body); font-size: 0.875rem;
...
.btn-borrar {
  ...
  background: rgba(13,15,20,0.85); border: 1px solid var(--c-border);
...
.form-perfil { display: flex; flex-direction: column; gap: 1rem; max-width: 560px; }
.campo { display: flex; flex-direction: column; gap: 0.35rem; }
.campo label { font-size: 0.8rem; color: var(--c-text-muted); }
...
.campo input:focus, .campo textarea:focus { border-color: var(--c-gold); }
...
.encargo-budget { font-size: 0.8rem; color: var(--c-gold); }
...
.badge-estado.pending { background: rgba(201,168,76,0.15); color: var(--c-gold); }
.badge-estado.accepted { background: rgba(80,200,120,0.15); color: #50c878; }
.badge-estado.rejected { background: var(--c-danger-dim); color: var(--c-danger); }
.btn-aceptar, .btn-rechazar {
  padding: 0.3rem 0.75rem; border-radius: var(--r-sm); font-size: 0.8rem;
  font-family: var(--font-body); cursor: pointer; transition: all 0.2s; border: 1px solid;
}
.btn-aceptar { background: rgba(80,200,120,0.1); border-color: rgba(80,200,120,0.3); color: #50c878; }
.btn-rechazar { background: var(--c-danger-dim); border-color: var(--c-danger-dim); color: var(--c-danger); }

.btn-primary {
  background: var(--c-gold); color: #0d0f14; border: none;
